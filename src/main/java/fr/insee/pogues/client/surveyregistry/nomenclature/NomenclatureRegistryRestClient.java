...
import fr.insee.pogues.client.surveyregistry.exceptions.ResourceRegistryNotFound;
...
@Slf4j
@Service
public class NomenclatureRegistryRestClient implements NomenclatureRegistryClient {
    ...
    public NomenclatureRegistryRestClient(@Qualifier("nomenclatureRegistryApiRestClient") RestClient restClient) {
    ...
    public List<NomenclatureDTO> getNomenclatures() throws HttpClientErrorException, HttpServerErrorException {
        URI uri = UriComponentsBuilder
                ...
                .queryParam(EXPAND_PARAM, SEARCH_CONFIGURATION_EXPAND_PARAM)
    ...
    public NomenclatureDTO getNomenclatureMetadataById(UUID nomenclatureId) {
                ...
                .onStatus(
                        ...
                        (request, response) -> {
                            if (response.getStatusCode().value() == 404) {
                                throw new ResourceRegistryNotFound(
                                        "Nomenclature " + nomenclatureId + NOT_FOUND);
                            }
        ...
        if (codesListsMetadata == null)
            throw new ResourceRegistryNotFound("Nomenclature " + nomenclatureId + NOT_FOUND);
